...
class AdminPanel:
    ...
    def get_summary_stats(self):
        try:
            with sqlite3.connect(self.db_name) as conn:
                ...
                c.execute("SELECT SUM(fine) FROM borrowings")
                total_fines = c.fetchone()[0] or 0
                return {"books": total_books, "users": total_users, "fines": total_fines}
        except sqlite3.Error as e:
            print(f"Error querying summary stats: {e}")
            return {"books": 0, "users": 0, "fines": 0}
    ...
    def refresh_book_list(self, search_term=""):
        print(f"Refreshing book list with search term: '{search_term}'")
        ...
        try:
            ...
            print(f"Database error in refresh_book_list: {e}")
    ...
    def view_fines(self):
        try:
            ...
            fine_win.attributes('-fullscreen', True)
            fine_win.configure(bg="#e9ecef")

            left_panel = tk.Frame(fine_win, bg="#e9ecef", width=200, relief="flat", highlightbackground="#ced4da", highlightthickness=1)
            left_panel.pack(side="left", fill="y")
            right_panel = tk.Frame(fine_win, bg="#e9ecef", width=200, relief="flat", highlightbackground="#ced4da", highlightthickness=1)
            right_panel.pack(side="right", fill="y")

            main_frame = tk.Frame(fine_win, bg="#e9ecef")
            main_frame.pack(expand=True, fill="both", padx=40, pady=40)

            tree = ttk.Treeview(main_frame, columns=("ID", "Username", "Book", "Due Date", "Return Date", "Fine"), show="headings")
            ...
            tk.Label(main_frame, text=f"Total Fines Outstanding: Rs. {total_fine}", font=("Segoe UI", 12, "bold"), bg="#e9ecef", fg="#007bff").pack(pady=10)
        except sqlite3.Error as e:
    ...
    def view_users(self):
        try:
            ...
            user_win.attributes('-fullscreen', True)
            user_win.configure(bg="#e9ecef")

            left_panel = tk.Frame(user_win, bg="#e9ecef", width=200, relief="flat", highlightbackground="#ced4da", highlightthickness=1)
            left_panel.pack(side="left", fill="y")
            right_panel = tk.Frame(user_win, bg="#e9ecef", width=200, relief="flat", highlightbackground="#ced4da", highlightthickness=1)
            right_panel.pack(side="right", fill="y")

            main_frame = tk.Frame(user_win, bg="#e9ecef")
            main_frame.pack(expand=True, fill="both", padx=40, pady=40)

            tk.Label(main_frame, text="All Users", font=("Segoe UI", 16, "bold"), bg="#e9ecef", fg="#007bff").pack(anchor="center", padx=15, pady=10)
            user_tree = ttk.Treeview(main_frame, columns=("Username", "Is Admin"), show="headings")
            ...
            user_scrollbar = ttk.Scrollbar(main_frame, orient="vertical", command=user_tree.yview)
            ...
            tk.Label(main_frame, text="Borrowing History", font=("Segoe UI", 16, "bold"), bg="#e9ecef", fg="#007bff").pack(anchor="center", padx=15, pady=10)
            borrow_tree = ttk.Treeview(main_frame, columns=("ID", "Username", "Book", "Borrow Date", "Due Date", "Return Date", "Fine"), show="headings")
            ...
            borrow_scrollbar = ttk.Scrollbar(main_frame, orient="vertical", command=borrow_tree.yview)
            ...
            tk.Label(main_frame, text=f"Total Fines Across All Users: Rs. {total_fine}", font=("Segoe UI", 12, "bold"), bg="#e9ecef", fg="#007bff").pack(pady=10)
        except sqlite3.Error as e:
    ...
    def update_summary(self):
        ...
        tk.Label(self.summary_frame, text="Library Summary", font=("Segoe UI", 16, "bold"), bg="white", fg="#007bff").pack(pady=10)
        tk.Label(self.summary_frame, text=f"Total Books: {stats['books']}", font=("Segoe UI", 12), bg="white", fg="#20c997").pack()
        tk.Label(self.summary_frame, text=f"Total Users: {stats['users']}", font=("Segoe UI", 12), bg="white", fg="#007bff").pack()
        tk.Label(self.summary_frame, text=f"Total Fines: Rs. {stats['fines']}", font=("Segoe UI", 12), bg="white", fg="#dc3545").pack()
    ...
    def setup_gui(self):
        ...
        # Summary Card
        self.summary_frame = tk.Frame(main_container, bg="white", padx=25, pady=20, relief="flat", highlightbackground="#20c997", highlightthickness=2)
        self.summary_frame.pack(fill="x", pady=15, padx=100, anchor="center")
        self.update_summary()

        # Search Bar
        search_frame = tk.Frame(main_container, bg="#e9ecef")
        search_frame.pack(fill="x", pady=15, anchor="center")
        tk.Label(search_frame, text="Search Books:", font=("Segoe UI", 12, "bold"), bg="#e9ecef").pack(side="left", padx=15)
        self.search_var = tk.StringVar()
        search_entry = tk.Entry(search_frame, textvariable=self.search_var, font=("Segoe UI", 12), width=35)
        search_entry.pack(side="left", padx=15)
        search_entry.bind("<KeyRelease>", lambda event: self.refresh_book_list(self.search_var.get()))

        # Add Book Form
        form_frame = tk.Frame(main_container, bg="white", padx=25, pady=20, relief="flat", highlightbackground="#20c997", highlightthickness=3)
        form_frame.pack(fill="x", pady=15, padx=100, anchor="center")
        ...
        # Book List
        canvas_frame = tk.Frame(main_container, bg="#e9ecef")
        canvas_frame.pack(fill="both", expand=True, pady=15, padx=100, anchor="center")
        ...
        # Status Bar
        status_frame = tk.Frame(main_container, bg="#e9ecef")
        status_frame.pack(fill="x", pady=10)
        tk.Label(status_frame, text=f"Last Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", font=("Segoe UI", 10), bg="#e9ecef", fg="#6c757d").pack(side="left", padx=15)
